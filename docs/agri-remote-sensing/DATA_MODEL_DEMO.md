...
已实现（需 **Bearer**；详见 [FRONTEND_API_AGRI.md](./FRONTEND_API_AGRI.md)）：

| 本文档概念 | 接口 |
|------------|------|
| 整包 `meta` + `parcels` + `timeseries` | `GET /api/v1/agri/demo-bundle` |
| 仅地块 GeoJSON | `GET /api/v1/agri/parcels` |
| 单地块时序 | `GET /api/v1/agri/parcels/{parcel_code}/timeseries`（`parcel_code` 如 `p1`） |
| 区域列表（取 `region_id`） | `GET /api/v1/agri/regions` |

后续可加分页、`org_id` 过滤、多指数等，见 [REQUIREMENTS.md](./REQUIREMENTS.md) §3.1、§9。
...
- [FRONTEND_API_AGRI.md](./FRONTEND_API_AGRI.md)（已实现接口）
